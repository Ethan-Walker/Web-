...
function PublishCampus(publish) {
...
PublishCampus.prototype.save = function(callback) {
    ...
    mongodb.open(function(error, db) {
        ...
        db.collection('publishCampus', function(err, collection) {
            if (err) {
                mongodb.close();
                return callback(err);
            }
...
PublishCampus.getAll = function(username, callback) {
...
PublishCampus.getOne = function(author, second, title, callback) {
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
        }
...
//返回原始发表的内容
PublishCampus.edit = function(name, day, title, callback) {
    //打开数据库
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
...
//更新一篇文章及其相关信息
PublishCampus.update = function(name, day, title, post, callback) {
    //打开数据库
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
...
//删除一篇文章
PublishCampus.remove = function(name, second, title, callback) {
    //打开数据库
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
