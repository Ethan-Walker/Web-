...
PublishFun.prototype.save = function(callback) {
    ...
    mongodb.open(function(error, db) {
        ...
        db.collection('publishFun', function(err, collection) {
            if (err) {
                mongodb.close();
                return callback(err);
            }
...
PublishFun.getAll = function(username, callback) {
...
PublishFun.getOne = function(author, second, title, callback) {
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
        }
...
//返回原始发表的内容
PublishFun.edit = function(name, day, title, callback) {
    //打开数据库
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
...
//更新一篇文章及其相关信息
PublishFun.update = function(name, day, title, post, callback) {
    //打开数据库
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
...
//删除一篇文章
PublishFun.remove = function(name, second, title, callback) {
    //打开数据库
    mongodb.open(function(err, db) {
        if (err) {
            return callback(err);
